...
#define readPortC(x)		((GPIOC_PDIR &= (1 << x)) >> x)
...
{	
	...
	for(;;)
    {
		 	...
		 	bottonPause = readPortC(8);
		 	//bottonPlay = readPortC(9);
			...
			sendCode(nIns, 0x87);
			sendCode(nData, (contadorCentenas/100) + 0x30);
			sendCode(nData, (contadorDecenas/10) + 0x30);
			sendCode(nData, contadorUnidades + 0x30);
			...
			if(contadorUnidades > 9)
			{
				contadorUnidades = 0;
				sendCode(nIns, 0x89);
				sendCode(nData, (0) + 0x30);
			}
			if(contadorDecenas > 99)
			{
				contadorDecenas = 0;
			}
			if(contadorUnidades > 999)
			{
				contadorDecenas = 0;
			}
    } 
...
void cfgPorts(void)
{
    ...
    GPIOB_PDDR = 0xFFFFFFFF; //1111 1111 1111 1111 1111 1100 1111 1111
     
    //Configure all PortC as outputs
    GPIOC_PDDR = 0xFFFFFCFF;
